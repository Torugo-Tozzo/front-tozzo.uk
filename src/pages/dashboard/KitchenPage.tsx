...
function KitchenColumn({ stage, title, orders, onInfo }: { stage: Stage; title: string; orders: KitchenOrder[]; onInfo: (order: KitchenOrder, item: KitchenItem) => void }) {
...
export default function KitchenPage() {
  ...
  const grouped = useMemo(() => columns.map(column => ({ ...column, orders: orders.map(order => {
    const byProduct = new Map<string, KitchenItem>()
    order.items.filter(item => item.stage === column.stage).forEach(item => {
  ...
  const handleDragEnd = async ({ active, over }: DragEndEvent) => { if (!over || !active.data.current?.item || !active.data.current?.order || over.id === active.data.current.item.stage) return; const item = active.data.current.item as KitchenItem, order = active.data.current.order as KitchenOrder; try { await api.patch(`/cozinha/pedidos/${order.id}/items/${item.id}`, { action: 'MOVE', itemIds: item.itemIds, targetStage: over.id, expectedUpdatedAt: order.updatedAt }); await load() } catch { setRefreshError('A comanda mudou. A fila será atualizada pelo SSE.') } }

...
test('starts a ready order without a driver', async () => {
  await act(async () => { await i18n.changeLanguage('pt-BR') })
  const auth = vi.spyOn(AuthContext, 'useAuth').mockReturnValue({ user: { role: 'OWNER', establishment: { tradeName: 'Bistro' } } } as ReturnType<typeof AuthContext.useAuth>)
  const realtime = vi.spyOn(RealtimeHooks, 'useRealtimeEvents').mockImplementation(() => undefined)
  restores.push(() => auth.mockRestore(), () => realtime.mockRestore())
  ...
  render(<I18nProvider><ConfirmProvider><DeliveriesPage /></ConfirmProvider></I18nProvider>)
  const start = await screen.findByRole('button', { name: 'Saiu para entrega' })
  expect(start).toBeEnabled()
  await act(async () => { fireEvent.click(start) })
  expect(api.patch).toHaveBeenCalledWith('/entregas/pedidos/order-1', expect.objectContaining({ action: 'START' }))
})

test('asks before marking pending items ready and starting delivery', async () => {
  await act(async () => { await i18n.changeLanguage('pt-BR') })
  const auth = vi.spyOn(AuthContext, 'useAuth').mockReturnValue({ user: { role: 'OWNER', establishment: { tradeName: 'Bistro' } } } as ReturnType<typeof AuthContext.useAuth>)
  const realtime = vi.spyOn(RealtimeHooks, 'useRealtimeEvents').mockImplementation(() => undefined)
  restores.push(() => auth.mockRestore(), () => realtime.mockRestore())
  restores.push(replaceProperty(api, 'get', vi.fn().mockResolvedValue({ data: { orders: [{ id: 'order-1', customerName: 'Ana', address: 'Rua A', total: 20, openedAt: '2026-09-23T12:00:00.000Z', updatedAt: '2026-09-23T12:00:00.000Z', stage: 'WAITING', driverId: null, driverName: null, items: [{ id: 'item-1', name: 'Burger', quantity: 1, status: 'IN_PREPARATION' }] }], drivers: [], totalPages: 1 } }) as typeof api.get))
  restores.push(replaceProperty(api, 'patch', vi.fn().mockResolvedValue({ data: {} }) as typeof api.patch))
  render(<I18nProvider><ConfirmProvider><DeliveriesPage /></ConfirmProvider></I18nProvider>)
  fireEvent.click(await screen.findByRole('button', { name: 'Saiu para entrega' }))
  const dialog = screen.getByRole('dialog')
  ...
  await act(async () => { fireEvent.click(within(screen.getByRole('dialog')).getByRole('button', { name: /confirmar/i })) })
  expect(api.patch).toHaveBeenCalledWith('/entregas/pedidos/order-1', expect.objectContaining({ action: 'START', confirmReady: true }))
})

test('explains a changed order after a failed start', async () => {
  await act(async () => { await i18n.changeLanguage('pt-BR') })
  const auth = vi.spyOn(AuthContext, 'useAuth').mockReturnValue({ user: { role: 'OWNER', establishment: { tradeName: 'Bistro' } } } as ReturnType<typeof AuthContext.useAuth>)
  const realtime = vi.spyOn(RealtimeHooks, 'useRealtimeEvents').mockImplementation(() => undefined)
  restores.push(() => auth.mockRestore(), () => realtime.mockRestore())
  restores.push(replaceProperty(api, 'get', vi.fn().mockResolvedValue({ data: { orders: [{ id: 'order-1', customerName: 'Ana', address: 'Rua A', total: 20, openedAt: '2026-09-23T12:00:00.000Z', updatedAt: '2026-09-23T12:00:00.000Z', stage: 'WAITING', driverId: null, driverName: null, items: [{ id: 'item-1', name: 'Burger', quantity: 1, status: 'READY' }] }], drivers: [], totalPages: 1 } }) as typeof api.get))
  restores.push(replaceProperty(api, 'patch', vi.fn().mockRejectedValue({ response: { data: { code: 'DELIVERY_CONFLICT' } } }) as typeof api.patch))
  render(<I18nProvider><ConfirmProvider><DeliveriesPage /></ConfirmProvider></I18nProvider>)
  const start = await screen.findByRole('button', { name: 'Saiu para entrega' })
  await act(async () => { fireEvent.click(start) })
  expect(screen.getByRole('alert')).toHaveTextContent('O pedido mudou')
})

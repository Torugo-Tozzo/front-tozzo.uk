...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      // --muted no light (96.1%) e quase igual ao fundo (100%) - some a olho
      // nu. No dark (17.5% vs fundo 8.6%) ja contrasta bem, mantido como
      // esta. Cinza mais forte so no light pra igualar a visibilidade.
      "inline-flex h-10 items-center justify-center rounded-md border border-foreground bg-[hsl(0,0%,85%)] dark:bg-muted p-1 text-muted-foreground",
      className
    )}
    {...props}
  />

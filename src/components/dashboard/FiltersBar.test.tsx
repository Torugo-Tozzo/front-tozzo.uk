...
describe('FiltersBar', () => {
  ...
  it('calls onFilter when the Buscar button is clicked', async () => {
    ...
    render(<FiltersBar customerName={{ value: '', onChange: vi.fn() }} onFilter={onFilter} />)
  ...
  it('calls the field onChange handler when typing', async () => {
    ...
    const user = userEvent.setup()
    render(<FiltersBar customerName={{ value: '', onChange }} onFilter={vi.fn()} />)
    await user.type(screen.getByLabelText('Cliente / Mesa'), 'a')
    expect(onChange).toHaveBeenCalledWith('a')
  })

  it('disables inputs and the button while isLoading', () => {
    render(<FiltersBar customerName={{ value: '', onChange: vi.fn() }} onFilter={vi.fn()} isLoading />)
  ...
  it('toggles the mobile filter panel via the expand/collapse button', async () => {
    const user = userEvent.setup()
    render(<FiltersBar customerName={{ value: '', onChange: vi.fn() }} onFilter={vi.fn()} />)
